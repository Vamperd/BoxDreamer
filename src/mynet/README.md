...
- `train/loss_step`：每个训练 step 的 8 通道 heatmap MSE，下降越稳定越好。
- `train/loss_epoch`：每个 epoch 的平均 heatmap MSE，用于看整体收敛。
- `val/loss`：验证集 heatmap MSE。
...
当前训练目标已经回退为直接 8 角点 heatmap 监督：

- 只对模型输出 `[B, 8, 64, 64]` 与目标 heatmap 计算 MSE。
- 不再使用 soft-argmax 坐标 loss。
- 不再区分可见/不可见角点损失；推荐数据集中 8 个角点均生成高斯 heatmap。
